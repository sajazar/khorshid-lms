...
final class Frontend {
    ...
    public static function course_shortcode( array $atts = [] ): string {
        ...
        if ( ! $access && 'paid' === $course->type ) {
            $url = is_user_logged_in() && function_exists( 'wc_get_cart_url' ) ? wc_get_cart_url() : wp_login_url( get_permalink() );
            $html .= '<a class="kh-button" href="' . esc_url( $url ) . '">' . esc_html( is_user_logged_in() ? 'خرید دوره' : 'ورود برای خرید' ) . '</a>';
        }
        ...
        foreach ( Repository::curriculum( $id ) as $chapter ) {
            ...
            foreach ( $chapter->lessons as $lesson ) {
                ...
                $html .= '<div class="kh-lesson-item"><span class="kh-lesson-icon">' . ( $locked ? '🔒' : '▶' ) . '</span><span>' . esc_html( $lesson->title ) . '</span><em>' . esc_html( $locked ? 'پس از خرید' : 'قابل مشاهده' ) . '</em></div>';
            }
            $html .= '</div></details>';
        }
    ...
    public static function my_courses_shortcode(): string {
        if ( ! is_user_logged_in() ) { return '<div class="kh-lms-notice">برای مشاهده دوره‌های خود وار�� شوید.</div>'; }
        global $wpdb;
