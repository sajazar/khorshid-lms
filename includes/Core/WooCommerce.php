...
final class WooCommerce {
    ...
    public static function maybe_show_missing_notice(): void {
        if ( current_user_can( 'manage_options' ) && ! self::available() ) {
            echo '<div class="notice notice-warning"><p>' . esc_html__( 'برای فروش دوره‌ها، نصب و فعال‌سازی WooCommerce لازم است.', 'khorshid-lms' ) . '</p></div>';
        }
    }
    ...
    public static function product_field(): void {
        ...
        foreach ( Repository::courses( [ 'status' => 'publish' ] ) as $course ) {
            $options[(string) $course->id] = $course->course_code . ' — ' . $course->title;
        }
        woocommerce_wp_select([
            'id' => '_kh_lms_course_id',
            'label' => __( 'دوره آموزشی', 'khorshid-lms' ),
            'description' => __( 'با خرید این محصول، دسترسی کاربر به دوره فعال می‌شود.', 'khorshid-lms' ),
            'desc_tip' => true,
            'options' => $options,
        ]);
    }
    ...
    public static function save_product_field( int $product_id ): void {
        if ( current_user_can( 'edit_post', $product_id ) && isset( $_POST['_kh_lms_course_id'] ) ) {
            update_post_meta( $product_id, '_kh_lms_course_id', absint( wp_unslash( $_POST['_kh_lms_course_id'] ) ) );
        }
    }
    ...
    public static function grant_access( int $order_id ): void {
        ...
        if ( ! $user_id ) { return; }
        global $wpdb;
        $table = Repository::table( 'enrollments' );
        ...
        foreach ( $order->get_items() as $item ) {
            ...
            $wpdb->query( $wpdb->prepare(
                'INSERT INTO ' . $table . ' (user_id,course_id,order_id,product_id,status,started_at,created_at) VALUES (%d,%d,%d,%d,%s,%s,%s) ON DUPLICATE KEY UPDATE status=%s,order_id=%d,product_id=%d',
                $user_id, $course_id, $order_id, $product_id, 'active', $now, $now, 'active', $order_id, $product_id
            ) );
        }
    }
    ...
    public static function revoke_access( int $order_id ): void {
        ...
        global $wpdb;
        foreach ( $order->get_items() as $item ) {
            $course_id = (int) get_post_meta( (int) $item->get_product_id(), '_kh_lms_course_id', true );
            if ( $course_id ) { $wpdb->update( Repository::table( 'enrollments' ), [ 'status' => 'revoked' ], [ 'user_id' => $user_id, 'course_id' => $course_id ], [ '%s' ], [ '%d', '%d' ] ); }
        }
    }
}
