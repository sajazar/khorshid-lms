...
/**
 * WooCommerce integration. The class is intentionally available even when
 * WooCommerce is not installed, so the LMS never causes a fatal error.
 */
final class WooCommerce {
    ...
    public static function product_field(): void {
        ...
        woocommerce_wp_text_input(
            [
                ...
                'label'       => __( 'شناسه دوره LMS', 'khorshid-lms' ),
                'description' => __( 'شناسه دوره‌ای که با خرید این محصول فعال می‌شود.', 'khorshid-lms' ),
                'desc_tip'    => true,
                'type'        => 'number',
    ...
    public static function save_product_field( int $product_id ): void {
        ...
        if ( ! isset( $_POST['_kh_lms_course_id'] ) ) {
            return;
        }
    ...
    public static function grant_access( int $order_id ): void {
        ...
        foreach ( $order->get_items() as $item ) {
            ...
            if ( ! $course_id || ! Repository::course( $course_id ) ) {
                continue;
            }

            $wpdb->query(
                $wpdb->prepare(
                    'INSERT INTO ' . $table . ' (user_id, course_id, order_id, product_id, status, started_at, created_at) VALUES (%d, %d, %d, %d, %s, %s, %s) ON DUPLICATE KEY UPDATE status = VALUES(status), order_id = VALUES(order_id), product_id = VALUES(product_id), started_at = COALESCE(started_at, VALUES(started_at))',
                    $user_id,
                    $course_id,
                    $order_id,
                    $product_id,
